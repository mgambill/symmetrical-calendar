...
const set = [
  primaryRoles.find((r) => r.permissionLevel === 'ADMIN') ?? { id: crypto.randomUUID(), name: 'xa', isActive: false },
  primaryRoles.find((r) => r.permissionLevel === 'WRITE') ?? { id: crypto.randomUUID(), name: 'xw', isActive: false },
  primaryRoles.find((r) => r.permissionLevel === 'READ') ?? { id: crypto.randomUUID(), name: 'xr', isActive: false },
]
  ...
  <section class="group grid grid-cols-subgrid col-span-full border-t *:p-4 border-white/60">
    ...
    <label class="text-center">
      <template v-if="primary === null">
        <i
          class="fa-duotone fa-solid fa-circle-check duo-primary-red-600 duo-secondary-red-100 dark:duo-primary-red-200 dark:duo-secondary-red-800"
        ></i>
      </template>
      <template v-else>
        <i
          data-inherited
          class="fa-duotone fa-solid fa-circle-x duo-primary-zinc-500 duo-secondary-zinc-100 dark:duo-primary-zinc-400 dark:duo-secondary-zinc-800"
        ></i>
      </template>

      <input type="radio" v-model="primary" :name="`${cat.id}`" :value="null" class="relative size-0 opacity-0 hidden" />
    </label>

    <template v-for="role in set" :key="role.id">
      <template v-if="role.isActive === false">
        <div class="text-center text-xs opacity-50 select-none">&mdash;</div>
      </template>
      <template v-else>
        <label class="text-center cursor-pointer">
          <template v-if="primary === role.id">
            <i
              data-checked
              class="fa-duotone fa-solid fa-circle-check duo-primary-green-600 duo-secondary-green-100 dark:duo-secondary-green-800 dark:duo-primary-green-500"
            ></i>
          </template>
          <template v-else-if="primaryRole && role && primaryRole?.depth <= role?.depth">
            <i
              data-inherited
              class="fa-duotone fa-solid fa-circle-half-horizontal duo-primary-green-600 duo-secondary-green-100 dark:duo-secondary-green-800 dark:duo-primary-green-500"
            ></i>
          </template>
          <template v-else>
            <i
